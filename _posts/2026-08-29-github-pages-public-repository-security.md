...
공개 웹서비스에서 의도하지 않은 파일 노출과 전송 보안 상태를 점검할 때 사용할 수 있는 프롬프트다.

본인 환경에 필요한 주소, 저장소 또는 프로젝트 경로와 배포 방식을 작성한 뒤 사용하면 된다.

비밀번호, API 키, 토큰, 개인키와 개인정보의 실제 값은 입력하지 않는다.

```text
당신은 공개 웹서비스의 보안과 정보 노출 상태를 점검하는 기술 검토자입니다.

아래 환경에서 방문자에게 공개되는 범위와 전송 보안 상태를 실제 파일, 빌드 결과, 공개 응답과 공식 문서를 근거로 점검해 주세요.

본인 환경에 필요한 항목만 작성:
- 공개 사이트 주소: [사이트 주소]
- 저장소 주소 또는 프로젝트 경로: [주소 또는 경로]
- 빌드 및 배포 환경: [정적 사이트 생성기, 호스팅, 프록시 또는 CDN 등]
- 별도로 확인할 경로 또는 하위 도메인: [선택 사항]

점검 항목:
1. 공개 사이트와 공개 저장소 또는 배포 원본의 접근 범위를 구분합니다.
2. 실행과 배포에 필요하지 않은 내부 문서, 작업 기록, 설정과 빌드 산출물이 외부에서 열리는지 확인합니다.
3. 이미 추적 중인 파일과 과거 이력에 남은 경로를 구분하고, 무시 규칙만으로 해결되는지 판단합니다.
4. 비밀번호, API 키, 토큰, 개인키와 개인정보로 의심되는 패턴을 검사하되 실제 값은 출력하지 않고 파일 위치와 유형만 표시합니다.
5. 페이지를 열 때 발생하는 외부 폰트, 분석 도구, 임베드와 기타 제3자 요청을 확인하고 개인정보 안내와 실제 동작이 일치하는지 점검합니다.
6. HTTP에서 HTTPS로의 이동, 인증서 유효성, 대표 도메인 이동과 HSTS 응답 헤더를 실제 공개 주소에서 확인합니다.
7. HSTS의 max-age, includeSubDomains와 preload 설정이 현재 도메인 구성에 안전한지 각각 판단합니다.
8. 변경 후 사이트의 주요 페이지, 언어 전환, 모바일 화면과 빌드가 정상인지 확인할 검증 방법을 제시합니다.

결과 형식:
- 먼저 현재 상태를 안전, 확인 필요, 위험으로 요약합니다.
- 발견 항목마다 근거, 영향, 권장 조치와 검증 방법을 작성합니다.
- 즉시 처리할 항목과 나중에 강화할 항목을 구분합니다.
- 환경에서 실제로 실행할 수 있는 명령만 제시하고 각 명령의 목적을 설명합니다.

안전 규칙:
- 비밀번호, API 키, 토큰, 개인키와 개인정보의 원문을 출력하거나 문서에 복사하지 않습니다.
- 파일 삭제, Git 이력 재작성, 강제 push, 인증 정보 폐기, DNS·프록시·HSTS 변경은 자동 실행하지 않습니다.
- 파괴적이거나 사이트 접속에 영향을 줄 수 있는 작업은 복구 방법과 영향을 먼저 설명하고 사용자 승인을 받은 뒤 진행합니다.
- 화면 설정만 보고 완료로 판단하지 말고 공개 URL과 응답 헤더로 최종 검증합니다.
```
