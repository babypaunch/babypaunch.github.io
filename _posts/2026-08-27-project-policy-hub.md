...
```text
당신은 GitHub Pages의 사용자 지정 도메인과 다국어 정책 허브를 점검하는 웹 운영자입니다.

내 저장소와 공개 사이트를 함께 확인하고 다음 항목을 점검해 주세요.

- 루트 도메인과 www 도메인의 DNS 연결
- HTTP에서 HTTPS로의 영구 리디렉션
- 두 도메인을 포함하는 유효한 HTTPS 인증서
- GitHub Pages의 사용자 지정 도메인과 HTTPS 강제 사용 상태
- `/policies/[프로젝트]/[문서]/` 형식의 프로젝트별 주소 구조
- 한국어·영어 정책 목록과 실제 문서 링크의 일치
- 프로젝트별 개인정보처리방침과 필요한 정책 문서의 누락
- 각 정책의 최초 시행일, 변경일과 변경 이력 표시
- 모바일에서 긴 제목, 주소와 링크가 화면을 넘지 않는지

각 항목을 `정상`, `개선 필요`, `확인 불가`로 판정해 주세요.

문제가 있다면 원인, 수정할 DNS·GitHub Pages 설정 또는 저장소 파일, 최소 수정안과 공개 사이트 검증 방법을 제시해 주세요.

정책 내용의 법적 적합성을 단정하지 말고 관할 지역과 서비스 특성에 따라 전문가 확인이 필요한 항목을 구분해 주세요.

사이트 주소: [정책 허브 주소]
GitHub 저장소: [저장소 경로 또는 주소]
DNS 제공자: [예: Cloudflare]
