...
지지는 띠를 나타내는 글자로만 알고 있었는데, 전통 역법에서는 달과 시간의 구간, 계절과 방위 등에도 연결된다. 예를 들어 하루를 두 시간씩 나누면 자시(子時)는 `23:00~00:59`, 축시(丑時)는 `01:00~02:59`처럼 이어진다.
...
단순히 `10 × 12 = 120`개의 조합을 모두 쓰는 것은 아니다. 음양(陰陽)이 같은 간과 지가 순서대로 만나기 때문에 서로 다른 조합은 예순 개이고, 계해(癸亥) 다음에는 다시 갑자(甲子)로 돌아간다. 이것이 육십갑자(六十甲子)다.
...
앱에서는 선택한 출생지 시간대의 실제 절입 시각을 사용한다. 현재 계산 범위는 `1900~2100년`이며, 검증된 변환 자료가 없는 음력(陰曆)과 윤달(閏月)은 입력받지 않는다. 모르는 것을 아는 척 계산하지 않는 것도 정확성의 일부라고 생각했다.
