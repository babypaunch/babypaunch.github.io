...
글을 계속 쓰려면 단순히 조회수가 많은지만 보는 것보다 더 많은 정보가 필요했다.

어떤 주제와 유입 경로가 실제로 사람을 데려오는지 알고 싶었다.
...
GitHub Pages는 별도의 애플리케이션 서버가 없는 정적 사이트다.

그래도 분석 기능을 적용할 수 있다.

모든 페이지가 공유하는 Jekyll 레이아웃에 분석 코드를 한 번만 넣으면 홈, 블로그, 음악, 정책 페이지에 같은 기준이 적용된다.
...
분석 도구를 실제로 사용하게 됐으므로 한국어와 영어 개인정보처리방침도 함께 고쳤다. 어떤 정보가 측정되는지, 거부해도 기능에 영향이 없는지, 선택을 어디서 바꿀 수 있는지를 두 언어에 같은 내용으로 적었다. 이 글을 처음 썼을 때는 Google AdSense 광고를 사용하지 않았다.

## Search Console로 검색 유입 연결하기

GA4는 사이트 안에서 일어나는 일을 보여준다.

Search Console은 Google 검색에서 사이트가 어떻게 발견되는지 보여준다.
...
소유권 확인이 끝난 뒤 `https://babypaunch.com/sitemap.xml`을 제출했다.

처음에는 잠시 가져올 수 없다는 상태가 보였다.

공개 주소가 HTTP `200`을 반환하고 XML에 문제가 없는지 확인한 뒤 다시 확인했다.

상태는 <strong>성공</strong>으로 바뀌었고, Search Console이 발견한 페이지는 27개였다.
...
2026년 9월 2일 Google AdSense 연결 코드를 추가하면서 광고 동의는 Google 인증 CMP가 별도로 담당하도록 정했고, 개인정보처리방침도 함께 갱신했다. B형간염 글에는 광고 코드를 적용하지 않는다.
