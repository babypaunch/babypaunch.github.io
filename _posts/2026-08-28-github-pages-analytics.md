...
Google Analytics 4에 `BabyPaunch Website` 속성과 `https://babypaunch.com` 웹 스트림을 만들었다. 사이트에는 공개 측정 ID `G-TFR80P4CNK`를 연결했다.
...
배포 후에는 실제 블로그 글을 열어 GA4 실시간 보고서를 확인했다. 대한민국에서 들어온 활성 사용자가 바로 표시됐다. 코드가 HTML에 들어갔다는 사실만 확인한 것이 아니라, 실제 공개 사이트에서 데이터가 도착하는 것까지 확인한 셈이다.

## 동의하기 전에는 분석 코드를 불러오지 않기

분석이 필요하다고 해서 방문자의 선택보다 먼저 실행하고 싶지는 않았다.
...
선택은 브라우저의 로컬 저장소에 보관한다. 나중에 마음이 바뀌면 페이지 아래의 <strong>분석 설정</strong>을 눌러 다시 선택할 수 있다. 허용하거나 거부했을 때 나타나는 짧은 안내는 2초 뒤 자동으로 사라진다.

모달은 화면 중앙에 두고 배경을 어둡게 처리했다. 키보드 포커스가 모달 밖으로 빠져나가지 않게 했고, 중요한 선택이므로 배경 클릭이나 `Esc`만으로 닫히지 않게 했다.

화면 크기도 감으로 확인하지 않았다. BabyPaunch의 공통 기준인 `640px`, `641px`, `1280px`, `1281px`에서 가로 넘침이 없는지 각각 검사했다.
...
코드를 배포하기 전에는 Jekyll `3.8`로 사이트를 빌드하고 25페이지 품질 테스트를 통과했다. 동의 전 미로드, 허용 후 로드, 거부, 설정 다시 열기와 네 개의 반응형 경계값도 실제 브라우저에서 확인했다. 변경 내용은 `main`에 커밋하고 push한 뒤 공개 사이트에서 다시 검증했다.
...
<p class="article-summary"><strong>한 줄 요약:</strong> 글을 많이 쓰는 것에서 멈추지 않고, 방문자의 선택을 먼저 지키면서 어떤 글과 유입 경로가 도움이 되는지 확인할 기반을 만들었다.</p>
